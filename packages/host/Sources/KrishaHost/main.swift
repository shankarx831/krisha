...
let dspProcessor = DSPProcessor(sampleRate: KrishaConfig.defaultSampleRate)
...
func main() {
    ...
    KrishaConfig.activeSampleRate = deviceSampleRate
    ...
    print("    ✓ Control file: \(KrishaConfig.controlFilePath)")
    print("    ✓ Preset file: \(KrishaConfig.presetFilePath)")
    ...
    Thread.sleep(forTimeInterval: KrishaConfig.deviceWaitTimeout)
    ...
    if KrishaConfig.activeSampleRate != KrishaConfig.defaultSampleRate {
        if dspProcessor.setSampleRate(KrishaConfig.activeSampleRate) {
            print("    DSP sample rate updated to \(KrishaConfig.activeSampleRate) Hz")
        } else {
            print("    WARNING: Failed to update DSP sample rate, using \(KrishaConfig.defaultSampleRate) Hz")
        }
    }
...
func cleanup() {
    ...
    unlink(KrishaConfig.controlFilePath)

    Thread.sleep(forTimeInterval: KrishaConfig.cleanupWaitTimeout)
