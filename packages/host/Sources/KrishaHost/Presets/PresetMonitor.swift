...
class PresetMonitor {
    ...
    private let queue = DispatchQueue(label: "com.krisha.preset-monitor")
    ...
    func startMonitoring() {
        ...
        let presetFilePath = KrishaConfig.presetFilePath
    ...
    private func checkForChanges() {
        if let attributes = try? FileManager.default.attributesOfItem(
            atPath: KrishaConfig.presetFilePath
        ),
    ...
    private func loadAndApplyPreset() {
        do {
            let preset = try loader.load(from: KrishaConfig.presetFilePath)
