...
class PresetLoader {
    func load(from path: String) throws -> krisha_preset_t {
    ...
    private func copyName(_ name: String, to preset: inout krisha_preset_t) {
    ...
    private func copyBands(_ bands: [BandJSON], to preset: inout krisha_preset_t) {
        for (i, band) in bands.prefix(10).enumerated() {
            withUnsafeMutablePointer(to: &preset.bands) { bandsPtr in
                let bandPtr = UnsafeMutableRawPointer(bandsPtr)
                    .advanced(by: i * MemoryLayout<krisha_band_t>.stride)
                    .assumingMemoryBound(to: krisha_band_t.self)

                bandPtr.pointee.frequency_hz = band.frequencyHz
                bandPtr.pointee.gain_db = band.gainDb
                bandPtr.pointee.q_factor = band.qFactor
                bandPtr.pointee.type = krisha_filter_type_t(UInt32(band.filterType))
