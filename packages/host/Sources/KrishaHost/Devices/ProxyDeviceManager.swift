...
class ProxyDeviceManager {
    ...
    private let volumeForwardQueue = DispatchQueue(label: "com.krisha.host.proxy-volume-forward")
    ...
    func findProxyDevice(forPhysicalUID physicalUID: String) -> AudioDeviceID? {
        let proxyUID = physicalUID + "-krisha"
    ...
    func autoSelectProxy() {
        ...
        if name.contains("Krisha") {
            ...
            if let physicalUID = uid.components(separatedBy: "-krisha").first,
    ...
    func handleProxySelection(_ proxyUID: String, deviceID: AudioDeviceID) {
        if let physicalUID = proxyUID.components(separatedBy: "-krisha").first,
           let physicalDevice = registry.find(uid: physicalUID) {
    ...
    func handlePhysicalSelection(_ physicalUID: String) {
        ...
        if let proxyID = findProxyDevice(forPhysicalUID: physicalUID) {
            ...
            print("Auto-switching to Krisha proxy")
            isAutoSwitching = true
            lastSwitchTime = now
            if setDefaultOutputDevice(proxyID) {
                activeProxyDeviceID = proxyID
    ...
    func restorePhysicalDevice() -> Bool {
        ...
        guard let currentDeviceID = getCurrentDefaultDevice(),
              ...
              name.contains("Krisha") else {
            return false
        }

        guard let proxyUID = getDeviceUID(currentDeviceID),
              let physicalUID = proxyUID.components(separatedBy: "-krisha").first,
        ...
        if result == noErr {
            ...
            Thread.sleep(forTimeInterval: KrishaConfig.physicalDeviceSwitchDelay)
            return true
        }
    ...
    func reregisterVolumeForwarding(attempt: Int = 1) {
        ...
        if monitoredVolumeElements.isEmpty {
            if attempt < KrishaConfig.wakeRetryMaxAttempts {
                let delay = KrishaConfig.wakeRetryDelays[attempt]
                print("[VolumeForward] Listener registration failed (attempt \(attempt)/\(KrishaConfig.wakeRetryMaxAttempts)) — retrying in \(delay)s")
                DispatchQueue.main.asyncAfter(deadline: .now() + delay) { [weak self] in
                    self?.reregisterVolumeForwarding(attempt: attempt + 1)
                }
            } else {
                print("[VolumeForward] ERROR: Listener registration failed after \(KrishaConfig.wakeRetryMaxAttempts) attempts")
            }
            return
        }
