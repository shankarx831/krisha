...
TEST(parser_basic) {
    ...
    krisha_error_t err = krisha_preset_parse_autoeq(autoeq_data, &preset);

    ASSERT_EQ(err, KRISHA_OK);
    ...
    ASSERT_EQ(preset.bands[0].type, KRISHA_FILTER_PEAK);
    ...
    ASSERT_EQ(preset.bands[1].type, KRISHA_FILTER_PEAK);
...
TEST(parser_varying_whitespace) {
    ...
    ASSERT(!preset.bands[1].enabled);
    ASSERT_EQ(preset.bands[1].type, KRISHA_FILTER_LOW_SHELF);
...
TEST(parser_missing_q_and_gain) {
    ...
    ASSERT_EQ(preset.bands[0].type, KRISHA_FILTER_HIGH_PASS);
